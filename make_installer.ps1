# Package dist\VRChronicle into a shareable VRChronicle-Setup.zip (run build.ps1 first).
$root = $PSScriptRoot
$name = "VRChronicle"
...
Copy-Item (Join-Path $root "installer\Install VRChronicle.cmd") $stage
...
Copy-Item (Join-Path $root "installer\Uninstall VRChronicle.cmd") (Join-Path $stage $name)
